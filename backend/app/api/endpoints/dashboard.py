...
def find_available_lines(
    db: Session, farm_id: Optional[int] = None
) -> List[Tuple[Line, Bed]]:
    """
    ...
    lines_query = (
        ...
        .filter(and_(Line.is_active == True, Bed.is_active == True))
    )
...
def calculate_planting_recommendations(
    db: Session, available_lines: List[Tuple[Line, Bed]], weeks_ahead: int
    ...
    upcoming_selling_dates = (
        ...
        .filter(
            and_(SellingSchedule.selling_date >= now, SellingSchedule.is_active == True)
        )
    ...
    recent_harvests = (
        db.query(Harvest).filter(Harvest.harvest_date >= now - timedelta(days=90)).all()
    )
    ...
    for line, bed in available_lines:
        ...
        for crop_name, growth_days in growth_cycles.items():
            ...
            seasonal_score = (
                1.0
                if crop_name in seasonal_preferences.get(current_season, [])
                else 0.5
            )
            ...
            current_plantings = (
                ...
                .filter(
                    and_(
                        Planting.crop_id
                        == db.query(Crop.id).filter(Crop.name == crop_name).scalar(),
                        Planting.is_active == True,
                    )
                )
                .count()
    ...
    recommendations.sort(
        key=lambda x: (x.priority == "high", x.market_demand_score), reverse=True
    )
...
def get_dashboard_data(
    ...
    try:
        ...
        planting_suggestions = calculate_planting_recommendations(
            db, available_lines, weeks_ahead
        )
