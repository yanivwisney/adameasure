...
def get_translations(language: str, db: Session = Depends(get_db)):
    ...
    translations = (
        ...
        .filter(Translation.language == language, Translation.is_active == True)
        .all()
    )
...
def create_bulk_translations(
    bulk_data: BulkTranslationCreate, db: Session = Depends(get_db)
):
...
def update_bulk_translations(
    bulk_update: BulkTranslationUpdate, db: Session = Depends(get_db)
):
...
def get_translations_by_category(
    category: str, language: str, db: Session = Depends(get_db)
):
