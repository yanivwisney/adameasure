...
from app.schemas.line import LineCreate, LineUpdate, Line as LineSchema, LineWithPlantings
...
def get_lines(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """Get all lines"""
    lines = db.query(Line).filter(Line.is_active == True).offset(skip).limit(limit).all()
    return lines
