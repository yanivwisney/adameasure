...
@router.post(
    "/", response_model=SellingScheduleSchema, status_code=status.HTTP_201_CREATED
)
def create_selling_schedule(
    schedule: SellingScheduleCreate, db: Session = Depends(get_db)
):
...
def get_selling_schedule(schedule_id: int, db: Session = Depends(get_db)):
    ...
    schedule = (
        db.query(SellingSchedule).filter(SellingSchedule.id == schedule_id).first()
    )
    if schedule is None:
...
def update_selling_schedule(
    schedule_id: int, schedule: SellingScheduleUpdate, db: Session = Depends(get_db)
):
