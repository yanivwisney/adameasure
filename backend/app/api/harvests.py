...
def update_harvest(
    harvest_id: int, harvest_update: HarvestUpdate, db: Session = Depends(get_db)
):
    ...
    if harvest_update.harvest_date or harvest_update.harvested_quantity:
        planting = (
            db.query(Planting).filter(Planting.id == db_harvest.planting_id).first()
        )
        ...
        if planting and planting.expected_harvest_date:
            ...
            db_harvest.days_early_late = (
                db_harvest.harvest_date - planting.expected_harvest_date
            ).days
        ...
        if crop and crop.expected_yield_per_plant and planting.quantity:
            ...
            if expected_yield > 0:
                db_harvest.yield_percentage = (
                    db_harvest.harvested_quantity / expected_yield
                ) * 100
...
def get_harvest_analytics(
    ...
    # Timing performance
    early_harvests = [
        h for h in harvests if h.days_early_late and h.days_early_late > 0
    ]
    ...
    # Yield performance
    high_yield = [
        h for h in harvests if h.yield_percentage and h.yield_percentage > 100
    ]
    low_yield = [h for h in harvests if h.yield_percentage and h.yield_percentage < 80]
    expected_yield = [
        h for h in harvests if h.yield_percentage and 80 <= h.yield_percentage <= 100
    ]
    ...
    return {
        ...
        "timing_performance": {
            ...
            "early_percentage": round(len(early_harvests) / total_harvests * 100, 1)
            if total_harvests > 0
            else 0,
            "late_percentage": round(len(late_harvests) / total_harvests * 100, 1)
            if total_harvests > 0
            else 0,
        },
        "yield_performance": {
            ...
            "high_yield_percentage": round(len(high_yield) / total_harvests * 100, 1)
            if total_harvests > 0
            else 0,
            "low_yield_percentage": round(len(low_yield) / total_harvests * 100, 1)
            if total_harvests > 0
            else 0,
        },
    }
