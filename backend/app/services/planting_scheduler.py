...
class PlantingScheduler:
    ...
    def generate_planting_suggestions(
        ...
        beds = (
            self.db.query(Bed)
            .filter(Bed.farm_id == farm_id, Bed.is_active == True)
            .all()
        )
        ...
        selling_dates = self._calculate_selling_dates(
            target_date, frequency_days, 12
        )  # 12 months ahead
    ...
    def _calculate_selling_dates(
        self, start_date: datetime, frequency_days: int, months_ahead: int
    ) -> List[datetime]:
    ...
    def _suggest_plantings_for_date(
        self, selling_date: datetime, beds: List[Bed], crops: List[Crop], farm_id: int
    ) -> List[PlantingSuggestion]:
        ...
        for crop in crops:
            ...
            if planting_date > datetime.now():
                ...
                if self._is_crop_suitable_for_date(crop, planting_date):
                    # Find available beds
                    available_beds = self._find_available_beds(
                        beds, planting_date, farm_id
                    )
    ...
    def _find_available_beds(
        self, beds: List[Bed], planting_date: datetime, farm_id: int
    ) -> List[Bed]:
        ...
        for bed in beds:
            ...
            conflicting_plantings = (
                self.db.query(Planting)
                .filter(
